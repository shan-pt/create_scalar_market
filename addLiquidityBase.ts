...
// Seer protocol router (you'll need to find the correct address for Base)
const SEER_BASE_ROUTER = getAddress(
  "0xeC9048b59b3467415b1a38F63416407eA0c70fB8" // This might need to be updated for Base
);
...
const USDC_ADDRESS = getAddress("0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913"); // USDC on Base
...
export async function addLiquidityBase(
  marketAddress: string,
  amount: number,
  lowerBound: number,
  upperBound: number
): Promise<void> {
  console.log("🚀 Adding liquidity to Seer prediction market on Base...");
  ...
  const amountWei = parseEther(amount.toString()).div(BigNumber.from(10).pow(18 - usdcDecimals));
  
  console.log(`💰 Current USDC balance: ${formatEther(usdcBalance.mul(BigNumber.from(10).pow(18 - usdcDecimals)))} USDC`);

  if (usdcBalance.lt(amountWei)) {
    throw new Error(`Insufficient USDC balance. Need ${amount} USDC, have ${formatEther(usdcBalance.mul(BigNumber.from(10).pow(18 - usdcDecimals)))} USDC`);
  }
  ...
  console.log(`      - Split amount: ${formatEther(splitAmount.mul(BigNumber.from(10).pow(18 - usdcDecimals)))} USDC`);
  console.log(`      - USDC per pool: ${formatEther(usdcPerPool.mul(BigNumber.from(10).pow(18 - usdcDecimals)))} USDC`);

  // Approve USDC for router
  console.log("Approving USDC for router...");
  const approveTx = await usdcContract.approve(SEER_BASE_ROUTER, splitAmount, {
    gasLimit: 100_000
  });
  await approveTx.wait();

  // Split position to get conditional tokens
  console.log(`Split ${formatEther(splitAmount.mul(BigNumber.from(10).pow(18 - usdcDecimals)))} USDC into conditional tokens using splitPosition`);
  ...
  console.log(`Token balances after split - DOWN: ${formatEther(downBalance)}, UP: ${formatEther(upBalance)}, USDC: ${formatEther(newUSDCBalance.mul(BigNumber.from(10).pow(18 - usdcDecimals)))}`);
  ...
  console.log(`\n✅ Successfully added liquidity to both pools on Base!`);
}
...
if (require.main === module) {
  ...
  if (args.length < 4) {
    console.log("Usage: npx ts-node addLiquidityBase.ts <marketAddress> <amount> <lowerBound> <upperBound>");
    console.log("Example: npx ts-node addLiquidityBase.ts 0x123... 100 0.3 0.7");
    process.exit(1);
  }
  ...
  addLiquidityBase(marketAddress, amount, lowerBound, upperBound)
    .then(() => {
